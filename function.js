  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const respid = await axios
        ...
        .then((r) => r.data.data);
      await axios.get(
        `https://william.wow.wrtn.ai/chat/v3/${chatid}/${respid}?model=gpt4&platform=web&user=${wrtnemail}&timestamp=${Date.now()}`,
        {
          headers: {
            authority: "william.wow.wrtn.ai",
...
//GPT CODE (TOO LAZY TO IMPLEMENT, MAYBE NEXT UPDATE)
// async function WRTNNewChatAndSendMessage(msg, c) {
//   const gptver = c.toString();
//   if (!["4", "3.5", "3.5_16k"].includes(gptver))
//     throw new Error(`Invalid GPT ver, expect 4, 3.5, 3.5_16k, got ${gptver}`);
...
//             `https://william.wow.wrtn.ai/chat/v3/${chatid}/${respid}/result?timestamp=${Date.now()}`,
//             {
//               headers: {
//                 authority: "william.wow.wrtn.ai",
//                 accept: "application/json, text/plain, */*",
