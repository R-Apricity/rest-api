...
airouter.post("/animevoicegen", async (req, res) => {
  ...
  socket.onclose = () =>
    res.json({ response: link, data: { text, character, speed, language } });
  socket.onerror = (err) => {
    throw new Error(err);
  };
});
...
airouter.post("/chatgpt", async (req, res) => {
   ...
   *         description: Client did not specify an auth token
   */
  const { gptver = "3.5_16k", text = "Who are you?" } = req.body.query;
  const resp = await func.WRTNNewChatAndSendMessage(text, gptver);
...
dlrouter.post("/youtube", async (req, res, next) => {
  /**
   * @swagger
   ...
   *               query:
   *                 type: string
   *                 example: https://www.youtube.com/watch?v=rRHtCp5-ENo&pp=ygUQYXJpZ2F0b3Ugbm8gaGFuYQ%3D%3D
   *     responses:
   *       200:
   *         description: Server Successful processed the request
   *       400:
  ...
  if(!query.match(/^(?:https?:\/\/)?(?:m\.|www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/)) {
    return res.status(400).json({error: "Invalid youtube url"});
  }
  await youtubedlv2(query)
    .catch((dt) => {
      return res.json({ error: dt });
    })
    .then((dt) => {
      return res.json({ response: dt });
    });
});
...
form.append('files[]', await spotify.downloadTrack(query), 'track.mp3');
...
const form = new FormData();
form.append('files[]', image, 'captcha.jpg');
...
const {data} = await axios.post(
  ...
  res.json({ response: {text: text, image: data.files[0].url}});
});
