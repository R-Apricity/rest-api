...
let voicegenurl =
  process.env.VCGEN_URL || "plachta-vits-umamusume-voice-synthesizer.hf.space";
...
airouter.post("/chatgpt", async (req, res) => {
   ...
   *         description: Client did not specify an auth token
   */
    const { model, text, chatid, generateImages } = req.body.query;
    const resp = await func.WRTNNewChatAndSendMessage(
      text || "Who are you?",
      model || "gpt3.5",
      chatid,
      generateImages
    ...
    const mod = {
      ...
      data: { ...resp, model, text, generateImages },
    };
    if (generateImages) {
      console.log(resp.data)
      mod["images"] = resp.data.image.imageUrls;
    }
    res.json(mod);
  } catch (e) {
    res.status(500).json({
...
airouter.post("/gemini", async (req, res) => {
  ...
  if (data.promptFeedback.blockReason) {
    throw new Error(
      `Your request is blocked because ${data.promptFeedback.blockReason} concerns`
    );
  }
...
airouter.post("/characterai/:characterid", async (req, res) => {
  ...
  if (chatId) {
    await chat.changeToConversationId(chatId);
  }
  const res2 = await chat.sendAndAwaitResponse(query, true);
  res.json({
    response: res2.text,
    data: { ...res2 },
    chatId: res2.chat.externalId,
  });
});

airouter.post("/characteraisearch", async (req, res) => {
  /**
   * @swagger
   * /api/ai/characteraisearch:
   *   post:
   *     summary: Chat with Character AI
   *     tags:
   *       - AI
  ...
  const dt = await characterAI.searchCharacters(query);
  res.json({
    response: dt,
  });
});
...
airouter.post("/animevoicegen", async (req, res) => {
  try {
     ...
     *         description: Client did not specify an auth token
     */
    let { speed, character, language, text } = req.body.query;
    ...
    const socket = new WebSocket(`wss://${voicegenurl}/queue/join`);
    let sessionHash = Math.random().toString(36).substring(2);
    let link;

    socket.onopen = () => console.log(`Connected to ${voicegenurl} websocket`);
    ...
    socket.onmessage = async (event) => {
      ...
      switch (message["msg"]) {
        case "send_hash":
          socket.send(
            JSON.stringify({ session_hash: sessionHash, fn_index: 2 })
          );
          break;
        case "send_data":
          socket.send(
            JSON.stringify({
              fn_index: 2,
              data: [
                text || "To be honest, I have no idea what to say as examples.",
                character,
                language,
                Number(speed),
                false,
        ...
        case "process_completed":
          ...
          if (message.output.data[0] != "Success") {
            return res.status(500).json({ error: message.output.data[0] });
          }
          link =
            `https://${voicegenurl}/file=` + message["output"].data[1].name;
          break;
      }
    };
    socket.onerror = (err) => {
    ...
    socket.onclose = async () => {
      const lonk = await func.Pomfup(
        await func.convertWavToMp3(
          await axios
      ...
      res.json({
        response: lonk,
        data: {
          text: text || "To be honest, I have no idea what to say as examples.",
          character,
          speed,
          language,
        },
...
// airouter.post("/vtubevoicegen", async (req, res) => {
//   try {
//     /**
//      * @swagger
//      * /api/ai/vtubevoicegen:
//      *   post:
//      *     summary: Generate AI vtube Voice.
//      *     tags:
//      *       - AI
//      *     parameters:
//      *       - in: query
...
dlrouter.post("/tiktok", async (req, res) => {
  ...
  // if (
  //   !query.match(
  //     /^https?:\/\/(?:m|www|vm)?\.?tiktok\.com\/(?:.*\b(?:usr|v|embed|user|video|photo)\/|\?shareId=|\&item_id=)(\d+|\w+)/
  //   )
  // )
  //   return res.status(400).json({ error: "Invalid tiktok url" });
  if (!["v1", "v2", "v3"].includes(version.toString()?.toLowerCase()?.trim()))
    version = "v3";
  const resp = await TTDL(query, { version: version });
...
dlrouter.post("/spotify", async (req, res) => {
  ...
  return res.json({
    ...
    response: await func.Pomfup(
      await spotify.downloadTrack(query),
      "track.mp3"
    ),
  });
});
...
utilityrouter.post("/generatecaptcha", async (req, res) => {
  ...
  res.json({
    response: {
      answer: text,
      captcha: await func.Pomfup(image, "captcha.jpg"),
    },
  });
});
...
utilityrouter.post("/youtubesearch", async (req, res) => {
   ...
   */
  let { query } = req.body;
  const data = await yts.GetListByKeyword(query)
  res.json({response: data?.items})
});

utilityrouter.post("/math", async (req, res) => {
  /**
   * @swagger
   * /api/utility/math:
   *   post:
   *     summary: solve simple math problem
   *     tags:
   *       - Utility
   *     parameters:
   *       - in: query
   ...
   *         description: Client did not specify an auth token
   */
  res.json({ response: await evaluate(req.body.query) });
});
...
utilityrouter.post("/gtts", async (req, res) => {
  ...
  const response = await googleTTS.getAudioBase64(text, {
    lang: language,
