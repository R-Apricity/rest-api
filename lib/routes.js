...
airouter.post("/sydney", async (req, res) => {
  const type = req.body.query.Variant || "Balanced"
  const ret = await sydneyAPI.sendMessage(req.body.query.Text, { type });
  res.json({response: ret.text, data: {...ret}})
});
...
airouter.post("/animevoicegen", async (req, res) => {
  ...
  if (!Object.keys(animeaivc).includes(body.character.toLowerCase().trim()))
    return res.status(400).send(`Invalid character name`);
  const socket = new WebSocket(
    "wss://plachta-vits-umamusume-voice-synthesizer.hf.space/queue/join"
  );
  socket.onopen = () => {
    console.log("Connected to umamusume websocket");
  };
  const ae = Math.random().toString(36).substring(2);
  let link;
  socket.onmessage = (event) => {
    switch (JSON.parse(event.data)["msg"]) {
      case "send_hash":
        socket.send(JSON.stringify({ "session_hash": ae, fn_index: 2 }));
        break;
      case "send_data":
        socket.send(
          JSON.stringify({
            ...
            data: [
              body.text,
              animeaivc[body.character.toLowerCase().trim()],
              body.language,
              Number(body.speed),
              false,
            ],
      ...
      case "process_completed":
        link = "https://plachta-vits-umamusume-voice-synthesizer.hf.space/file="+JSON.parse(event.data)["output"].data[1].name;
        break;
      default:
        break;
    }
...
airouter.post("/prodiaimagegen", async (req, res) => {
  const {positive_prompt, negative_prompt, model, steps, cfg, seed} = req.body.query;
  const url = await func.ProdiaImageGen(positive_prompt, negative_prompt, model, steps, cfg, seed)
