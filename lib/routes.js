...
airouter.post("/animevoicegen", async (req, res) => {
  ...
  socket.onclose = async() => {
    const lonk = await func.Pomfup(await axios.get(link, {responseType: "arraybuffer"}).then(r => r.data), "audio.mp3")
  console.log(lonk)
    console.log("done")
    res.json({ response: lonk, data: { text, character, speed, language } });}
});
