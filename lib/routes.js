...
router.use("/", async (req, res, next) => {
  if (req.method !== "POST")
    return res.status(400).json({error:"Forbidden, Use POST not " + req.method});
  if (!req.body.query)
    return res.status(400).json({error:"Missing query parameters on Body"});
  if (process.env.auth_enabled === "true") {
    if (!req.query.auth) return res.status(403).json({error: "Forbidden, No Auth Key"});
...
airouter.post("/sydney", async (req, res) => {
  ...
  if (!variant || !["Balanced", "Precise", "Creative"].includes(variant.toString().trim())) variant = "Balanced"
